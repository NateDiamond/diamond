...
export type Option = {name: any, title: string, icon: ReactElement};
...
export function Toggle({options, selectedOption, setSelectedOption, color, secondaryColor, brightness}: ToggleParams) {
  return (
    <div className={`flex flex-row h-12 border ${getColor("border", secondaryColor, brightness, true, false)} rounded-full ${getColor("bg", color, brightness)} ${getColor("text", secondaryColor, brightness)}`}>
      { options.map( (option) => {
        return (
          <div title={option.title} key={String(option.name)} className={`flex p-2 rounded-full ${getColor("text", option.name === selectedOption ? color : secondaryColor, brightness, true, option.name === selectedOption)} ${getColor("bg", option.name === selectedOption ? secondaryColor : color, brightness)} ${option.name === selectedOption ? `rounded-full border ${getColor("border", secondaryColor, brightness, true, false)}` : ''}`} onClick={() => setSelectedOption(option.name)}>
            {option.icon}
          </div>
        );
      })}
